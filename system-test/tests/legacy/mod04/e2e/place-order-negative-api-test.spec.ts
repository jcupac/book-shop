...
test('shouldRejectOrderWithNonIntegerQuantity', async ({ bookShopApiClient }) => {
    // When: place order with non-integer quantity via API client
    const result = await bookShopApiClient.orders().placeOrder({ sku: 'SOME-SKU', quantity: 'invalid-quantity', country: 'US' });
