...
test('shouldRejectOrderWithNonIntegerQuantity', async ({ bookShopUiClient }) => {
    // When: place order with invalid quantity via UI client
    const homeResult = await bookShopUiClient.openHomePage();
    expect(homeResult.success).toBe(true);
    if (!homeResult.success) return;
    await homeResult.value.clickNewOrder();

    const newOrderPage = bookShopUiClient.newOrderPage();
