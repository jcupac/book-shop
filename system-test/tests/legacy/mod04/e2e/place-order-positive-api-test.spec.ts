...
test('shouldPlaceOrderForValidInput', async ({ bookShopApiClient, erpClient }) => {
    ...
    const result = await bookShopApiClient.orders().placeOrder({ sku, quantity: '5', country: 'US' });
    ...
    if (result.success) {
        ...
        const viewResult = await bookShopApiClient.orders().viewOrder(result.value.orderNumber);
