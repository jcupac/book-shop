...
test('shouldPlaceOrderForValidInput', async ({ config }) => {
    ...
    const bookShopApiUrl = config.bookShop.backendApiUrl;
    ...
    const placeOrderResponse = await fetch(`${bookShopApiUrl}/api/orders`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    const viewOrderResponse = await fetch(`${bookShopApiUrl}/api/orders/${orderData.orderNumber}`);
