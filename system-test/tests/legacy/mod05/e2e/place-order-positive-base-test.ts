...
export function runPlaceOrderPositive(test: TestType<any, any>): void {
  test('shouldPlaceOrderForValidInput', async ({ bookShopDriver, erpDriver }) => {
    const sku = `SKU-${randomUUID().substring(0, 8)}`;

    const productResult = await erpDriver.returnsProduct({ sku, price: '20.00' });
    expect(productResult.success).toBe(true);

    const result = await bookShopDriver.placeOrder({ sku, quantity: '5', country: 'US' });

    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.value.orderNumber).toMatch(/^ORD-/);

      const viewResult = await bookShopDriver.viewOrder({ orderNumber: result.value.orderNumber });
