...
// A10 — every BookShopDriver operation takes a single *Request and returns Result<*Response, …> (strict).
test('A10 - every BookShopDriver method takes a *Request and returns a *Response', () => {
  const violations: string[] = [];
  let found = false;

  for (const sf of sourceFiles) {
    const iface = sf.getInterface('BookShopDriver');
    if (!iface) continue;
    found = true;
  ...
  expect(found, 'BookShopDriver interface not found').toBe(true);
  expect(violations, `Every BookShopDriver method must take a single *Request and return Result<*Response, …>: ${violations.join('; ')}`).toEqual([]);
});
