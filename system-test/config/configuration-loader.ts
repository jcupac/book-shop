...
export function loadConfiguration(overrides?: ConfigOverrides): TestConfig {
  ...
  config.bookShop.frontendUrl = getEnvVarOrDefault('MYSHOP_UI_BASE_URL' + suffix, config.bookShop.frontendUrl);
  config.bookShop.backendApiUrl = getEnvVarOrDefault('MYSHOP_API_BASE_URL' + suffix, config.bookShop.backendApiUrl);
