...
export class BookShopUiClient {
  ...
  async openHomePage(): Promise<Result<HomePage, SystemError>> {
    try {
      ...
      return failure({ message: `BookShop UI not available: ${response?.status()}`, fieldErrors: [] });
    } catch (e) {
      return failure({ message: `BookShop UI not available: ${e}`, fieldErrors: [] });
    }
  }
  ...
  private requirePage(): Page {
    if (!this.currentPage) {
      throw new Error('BookShopUiClient: openHomePage() must be called before accessing pages');
    }
