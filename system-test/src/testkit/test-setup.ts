...
import { BookShopApiDriver } from './driver/adapter/api/book-shop-api-driver.js';
import { BookShopUiDriver } from './driver/adapter/ui/book-shop-ui-driver.js';
...
export function createScenario(options: ScenarioOptions = {}): ScenarioDsl {
  ...
  const app = new AppContext({
    channelMode,
    channel,
    bookShopDriverFactory: (ch) => createBookShopDriverForChannel(config, ch as Channel, options),
...
function createBookShopDriverForChannel(config: TestConfig, channel: Channel, options: ScenarioOptions) {
  if (channel === ChannelType.UI) {
    if (!options.browser) throw new Error('Browser is required for UI channel');
    return new BookShopUiDriver(config.bookShop.frontendUrl, options.browser);
  }
