...
export class ThenCancelOrderResultStage implements PromiseLike<void> {
  ...
  private async _arrangeCoupons(): Promise<void> {
    for (const cc of this.ctx.couponConfigs) {
      ...
      await this.app.bookShop().publishCoupon({
        code: resolvedCode,
  ...
  private async _placeGivenOrders(): Promise<void> {
    for (const oc of this.ctx.orderConfigs) {
      ...
      const placeResult = await this.app.bookShop().placeOrder({
        sku: resolvedSku,
      ...
      if (oc.status === 'CANCELLED') {
        await this.app.bookShop().cancelOrder({ orderNumber: placeResult.value.orderNumber });
      }
      if (oc.status === 'DELIVERED') {
        await this.app.bookShop().deliverOrder({ orderNumber: placeResult.value.orderNumber });
      }
    }
  }
  ...
  private async _runOrderAssertions(targetOrderNumber: string): Promise<void> {
    ...
    const orderResult = await this.app.bookShop().viewOrder({ orderNumber: targetOrderNumber });
  ...
  private async _doExecute(): Promise<void> {
    ...
    const result = await this.app.bookShop('dynamic').cancelOrder({ orderNumber: targetOrderNumber });
